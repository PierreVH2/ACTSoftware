...
gboolean plc_stat_update(GIOChannel *plc_chan, GIOCondition condition, gpointer domemove);
void domemove_move(gpointer txv_console);
void domemove_stop(gpointer txv_console);
void domemove_fail(gpointer txv_console, const gchar *msg);
...
int main (int argc, char ** argv)
{
  ...
  GtkWidget *domemove = domemove_new (&plc_stat, plc_chan);
  gtk_table_attach(GTK_TABLE(box_main), domemove, 0, 3, 2, 3, GTK_FILL|GTK_EXPAND, GTK_SHRINK, 5, 5);
  g_signal_connect_swapped(G_OBJECT(domemove), "domemove-move", G_CALLBACK(domemove_move), txv_console);
  g_signal_connect_swapped(G_OBJECT(domemove), "domemove-stop", G_CALLBACK(domemove_stop), txv_console);
  g_signal_connect_swapped(G_OBJECT(domemove), "domemove-fail", G_CALLBACK(domemove_fail), txv_console);
  g_signal_connect_swapped(G_OBJECT(domemove), "proc-complete", G_CALLBACK(proc_complete), txv_console);
  g_signal_connect_swapped(G_OBJECT(domemove), "plc-comm-stat", G_CALLBACK(plc_comm_stat), txv_console);
  g_signal_connect_swapped(G_OBJECT(domemove), "user-interact", G_CALLBACK(user_interact), txv_console);
  ...
  g_object_set_data(send_msg_objects, "domemove", domemove);
  ...
  int plc_watch_id = g_io_add_watch(plc_chan, G_IO_IN, plc_stat_update, domemove);
...
void send_msg(gpointer user_data)
{
  ...
  GtkWidget *domemove = GTK_WIDGET(g_object_get_data(user_data, "domemove"));
  ...
  console_scroll_to_end(txv_console);
  domemove_process_msg(domemove, &msg);
}
...
gboolean plc_stat_update(GIOChannel *plc_chan, GIOCondition condition, gpointer domemove)
{
  ...
  domemove_update(GTK_WIDGET(domemove), &new_stat);
...
void domemove_move(gpointer txv_console)
{
  gtk_text_buffer_insert_at_cursor(gtk_text_view_get_buffer(GTK_TEXT_VIEW(txv_console)), "Dome moving\n", -1);
  console_scroll_to_end(txv_console);
}

void domemove_stop(gpointer txv_console)
{
  gtk_text_buffer_insert_at_cursor(gtk_text_view_get_buffer(GTK_TEXT_VIEW(txv_console)), "Dome stopped\n", -1);
  console_scroll_to_end(txv_console);
}
...
{
  char text[256];
  sprintf(text, "Dome move failure: %s\n", msg);
  gtk_text_buffer_insert_at_cursor(gtk_text_view_get_buffer(GTK_TEXT_VIEW(txv_console)), text ,-1);
  console_scroll_to_end(txv_console);
}
