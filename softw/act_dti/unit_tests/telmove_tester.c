...
gboolean plc_stat_update(GIOChannel *plc_chan, GIOCondition condition, gpointer telmove);
void telmove_moving(gpointer txv_console);
void telmove_stop(gpointer txv_console);
void telmove_track(gpointer txv_console, gboolean tracking_on);
void telmove_azm_change(gpointer txv_console, gfloat azm);
void telmove_fail(gpointer txv_console, const gchar *msg);
void proc_complete(gpointer txv_console, gint status);
void motor_comm_stat(gpointer txv_console, gboolean status);
...
int main (int argc, char ** argv)
{
  ...
  int plc_fd = open("/dev/" PLC_DEVICE_NAME, O_RDWR | O_NONBLOCK);
  ...
  char motordrv_stat;
  ret = read(motor_fd, &motordrv_stat, 1);
  if (ret < 0)
  {
    fprintf(stderr, "Failed to read motor driver status - %s (%d).\n", strerror(errno), errno);
    close(motor_fd);
    close(plc_fd);
    return 1;
  }
  ...
  gtk_table_attach(GTK_TABLE(box_main), btn_quit, 2, 3, 3, 4, GTK_FILL|GTK_EXPAND, GTK_SHRINK, 5, 5);
  
  GtkWidget *telmove = telmove_new(&plc_stat, motor_chan);
  gtk_table_attach(GTK_TABLE(box_main), telmove, 0, 3, 2, 3, GTK_FILL|GTK_EXPAND, GTK_SHRINK, 5, 5);
  g_signal_connect_swapped(G_OBJECT(telmove), "telescope-moving", G_CALLBACK(telmove_moving), txv_console);
  g_signal_connect_swapped(G_OBJECT(telmove), "telescope-stop", G_CALLBACK(telmove_stop), txv_console);
  g_signal_connect_swapped(G_OBJECT(telmove), "telescope-tracking", G_CALLBACK(telmove_track), txv_console);
  g_signal_connect_swapped(G_OBJECT(telmove), "telescope-azm-change", G_CALLBACK(telmove_azm_change), txv_console);
  g_signal_connect_swapped(G_OBJECT(telmove), "telescope-fail", G_CALLBACK(telmove_fail), txv_console);
  g_signal_connect_swapped(G_OBJECT(telmove), "proc-complete", G_CALLBACK(proc_complete), txv_console);
  g_signal_connect_swapped(G_OBJECT(telmove), "motor-comm-stat", G_CALLBACK(motor_comm_stat), txv_console);
  g_signal_connect_swapped(G_OBJECT(telmove), "user-interact", G_CALLBACK(user_interact), txv_console);
  ...
  g_object_set_data(send_msg_objects, "telmove", telmove);
  ...
  int plc_watch_id = g_io_add_watch(plc_chan, G_IO_IN, plc_stat_update, telmove);
...
void send_msg(gpointer user_data)
{
  ...
  GtkWidget *telmove = GTK_WIDGET(g_object_get_data(user_data, "telmove"));
  ...
  console_scroll_to_end(txv_console);
  telmove_process_msg(telmove, &msg);
}
...
gboolean plc_stat_update(GIOChannel *plc_chan, GIOCondition condition, gpointer telmove)
{
  ...
  telmove_update_handset(GTK_WIDGET(telmove), &new_stat);
...
void telmove_moving(gpointer txv_console)
{
  gtk_text_buffer_insert_at_cursor(gtk_text_view_get_buffer(GTK_TEXT_VIEW(txv_console)), "Telescope moving\n", -1);
  console_scroll_to_end(txv_console);
}

void telmove_stop(gpointer txv_console)
{
  gtk_text_buffer_insert_at_cursor(gtk_text_view_get_buffer(GTK_TEXT_VIEW(txv_console)), "Telescope stopped\n", -1);
  console_scroll_to_end(txv_console);
}
...
{
  char text[256];
  sprintf(text, "Telescope azimuth: %f\n", azm);
  gtk_text_buffer_insert_at_cursor(gtk_text_view_get_buffer(GTK_TEXT_VIEW(txv_console)), text, -1);
  console_scroll_to_end(txv_console);
}
...
{
  char text[256];
  sprintf(text, "Telescope failure %s\n", msg);
  gtk_text_buffer_insert_at_cursor(gtk_text_view_get_buffer(GTK_TEXT_VIEW(txv_console)), text, -1);
  console_scroll_to_end(txv_console);
}
...
{
  char text[256];
  sprintf(text, "Motor communications status: %s\n", status > 0 ? "OK" : "Fail");
  gtk_text_buffer_insert_at_cursor(gtk_text_view_get_buffer(GTK_TEXT_VIEW(txv_console)), text, -1);
  console_scroll_to_end(txv_console);
}
