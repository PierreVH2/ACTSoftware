...
gboolean plc_stat_update(GIOChannel *plc_chan, GIOCondition condition, gpointer filter);
void filter_cent(gpointer txv_console);
void filter_move(gpointer txv_console);
void proc_complete(gpointer txv_console, gint status);
void filter_fail(gpointer txv_console, const gchar *msg);
...
int main (int argc, char ** argv)
{
  ...
  GtkWidget *filter = filter_new (&plc_stat, plc_chan);
  gtk_table_attach(GTK_TABLE(box_main), filter, 0, 3, 2, 3, GTK_FILL|GTK_EXPAND, GTK_SHRINK, 5, 5);
  g_signal_connect_swapped(G_OBJECT(filter), "filter-cent", G_CALLBACK(filter_cent), txv_console);
  g_signal_connect_swapped(G_OBJECT(filter), "filter-move", G_CALLBACK(filter_move), txv_console);
  g_signal_connect_swapped(G_OBJECT(filter), "proc-complete", G_CALLBACK(proc_complete), txv_console);
  g_signal_connect_swapped(G_OBJECT(filter), "filter-fail", G_CALLBACK(filter_fail), txv_console);
  g_signal_connect_swapped(G_OBJECT(filter), "plc-comm-stat", G_CALLBACK(plc_comm_stat), txv_console);
  g_signal_connect_swapped(G_OBJECT(filter), "user-interact", G_CALLBACK(user_interact), txv_console);
  ...
  g_object_set_data(send_msg_objects, "filter", filter);
  ...
  int plc_watch_id = g_io_add_watch(plc_chan, G_IO_IN, plc_stat_update, filter);
...
void send_msg(gpointer user_data)
{
  ...
  GtkWidget *filter = GTK_WIDGET(g_object_get_data(user_data, "filter"));
...
gboolean plc_stat_update(GIOChannel *plc_chan, GIOCondition condition, gpointer filter)
{
  ...
  filter_update(GTK_WIDGET(filter), &new_stat);
...
void filter_cent(gpointer txv_console)
{
  gtk_text_buffer_insert_at_cursor(gtk_text_view_get_buffer(GTK_TEXT_VIEW(txv_console)), "Filter centred\n", -1);
}

void filter_move(gpointer txv_console)
{
  gtk_text_buffer_insert_at_cursor(gtk_text_view_get_buffer(GTK_TEXT_VIEW(txv_console)), "Filter moving\n", -1);
}
