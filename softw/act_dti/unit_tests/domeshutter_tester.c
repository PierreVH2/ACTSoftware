...
gboolean plc_stat_update(GIOChannel *plc_chan, GIOCondition condition, gpointer domeshutter);
void domeshutter_open(gpointer txv_console);
void domeshutter_closed(gpointer txv_console);
void domeshutter_move(gpointer txv_console);
void domeshutter_fail(gpointer txv_console, const gchar *msg);
...
void toggle_sensitive(GtkWidget *btn_sensitive, gpointer domeshutter);
...
int main (int argc, char ** argv)
{
  ...
  GtkWidget *domeshutter = domeshutter_new (&plc_stat, plc_chan);
  gtk_table_attach(GTK_TABLE(box_main), domeshutter, 0, 3, 3, 4, GTK_FILL|GTK_EXPAND, GTK_SHRINK, 5, 5);
  g_signal_connect_swapped(G_OBJECT(domeshutter), "domeshutter-open", G_CALLBACK(domeshutter_open), txv_console);
  g_signal_connect_swapped(G_OBJECT(domeshutter), "domeshutter-closed", G_CALLBACK(domeshutter_closed), txv_console);
  g_signal_connect_swapped(G_OBJECT(domeshutter), "domeshutter-move", G_CALLBACK(domeshutter_move), txv_console);
  g_signal_connect_swapped(G_OBJECT(domeshutter), "domeshutter-fail", G_CALLBACK(domeshutter_fail), txv_console);
  g_signal_connect_swapped(G_OBJECT(domeshutter), "proc-complete", G_CALLBACK(proc_complete), txv_console);
  g_signal_connect_swapped(G_OBJECT(domeshutter), "plc-comm-stat", G_CALLBACK(plc_comm_stat), txv_console);
  g_signal_connect_swapped(G_OBJECT(domeshutter), "user-interact", G_CALLBACK(user_interact), txv_console);
  
  GtkWidget *btn_sensitive = gtk_toggle_button_new_with_label("Sensitive");
  gtk_table_attach(GTK_TABLE(box_main), btn_sensitive, 0, 3, 2, 3, GTK_FILL|GTK_EXPAND, GTK_SHRINK, 5, 5);
  g_signal_connect(G_OBJECT(btn_sensitive), "toggled", G_CALLBACK(toggle_sensitive), domeshutter);
  ...
  g_object_set_data(send_msg_objects, "domeshutter", domeshutter);
  ...
  int plc_watch_id = g_io_add_watch(plc_chan, G_IO_IN, plc_stat_update, domeshutter);
...
void send_msg(gpointer user_data)
{
  ...
  GtkWidget *domeshutter = GTK_WIDGET(g_object_get_data(user_data, "domeshutter"));
  ...
  console_scroll_to_end(txv_console);
  domeshutter_process_msg(domeshutter, &msg);
}
...
gboolean plc_stat_update(GIOChannel *plc_chan, GIOCondition condition, gpointer domeshutter)
{
  ...
  domeshutter_update(GTK_WIDGET(domeshutter), &new_stat);
...
void domeshutter_open(gpointer txv_console)
{
  gtk_text_buffer_insert_at_cursor(gtk_text_view_get_buffer(GTK_TEXT_VIEW(txv_console)), "Domeshutter open\n", -1);
  console_scroll_to_end(txv_console);
}

void domeshutter_closed(gpointer txv_console)
{
  gtk_text_buffer_insert_at_cursor(gtk_text_view_get_buffer(GTK_TEXT_VIEW(txv_console)), "Domeshutter closed\n", -1);
  console_scroll_to_end(txv_console);
}

void domeshutter_move(gpointer txv_console)
{
  gtk_text_buffer_insert_at_cursor(gtk_text_view_get_buffer(GTK_TEXT_VIEW(txv_console)), "Domeshutter moving\n", -1);
  console_scroll_to_end(txv_console);
}
...
{
  char text[256];
  sprintf(text, "Domeshutter failure: %s\n", msg);
  gtk_text_buffer_insert_at_cursor(gtk_text_view_get_buffer(GTK_TEXT_VIEW(txv_console)), text ,-1);
  console_scroll_to_end(txv_console);
}
...
void toggle_sensitive(GtkWidget *btn_sensitive, gpointer domeshutter)
{
  domeshutter_set_sensitive(GTK_WIDGET(domeshutter), gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(btn_sensitive)));
}
