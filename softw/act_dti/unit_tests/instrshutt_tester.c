...
gboolean plc_stat_update(GIOChannel *plc_chan, GIOCondition condition, gpointer instrshutt);
void instrshutt_open(gpointer txv_console);
void instrshutt_shut(gpointer txv_console);
void instrshutt_fail(gpointer txv_console, const gchar *msg);
...
int main (int argc, char ** argv)
{
  ...
  GtkWidget *instrshutt = instrshutt_new (&plc_stat, plc_chan);
  gtk_table_attach(GTK_TABLE(box_main), instrshutt, 0, 3, 2, 3, GTK_FILL|GTK_EXPAND, GTK_SHRINK, 5, 5);
  g_signal_connect_swapped(G_OBJECT(instrshutt), "instrshutt-open", G_CALLBACK(instrshutt_open), txv_console);
  g_signal_connect_swapped(G_OBJECT(instrshutt), "instrshutt-closed", G_CALLBACK(instrshutt_shut), txv_console);
  g_signal_connect_swapped(G_OBJECT(instrshutt), "instrshutt-fail", G_CALLBACK(instrshutt_fail), txv_console);
  g_signal_connect_swapped(G_OBJECT(instrshutt), "proc-complete", G_CALLBACK(proc_complete), txv_console);
  g_signal_connect_swapped(G_OBJECT(instrshutt), "plc-comm-stat", G_CALLBACK(plc_comm_stat), txv_console);
  g_signal_connect_swapped(G_OBJECT(instrshutt), "user-interact", G_CALLBACK(user_interact), txv_console);
  ...
  g_object_set_data(send_msg_objects, "instrshutt", instrshutt);
  ...
  int plc_watch_id = g_io_add_watch(plc_chan, G_IO_IN, plc_stat_update, instrshutt);
...
void send_msg(gpointer user_data)
{
  ...
  GtkWidget *instrshutt = GTK_WIDGET(g_object_get_data(user_data, "instrshutt"));
  ...
  instrshutt_process_msg(instrshutt, &msg);
}
...
gboolean plc_stat_update(GIOChannel *plc_chan, GIOCondition condition, gpointer instrshutt)
{
  ...
  instrshutt_update(GTK_WIDGET(instrshutt), &new_stat);
...
void instrshutt_open(gpointer txv_console)
{
  gtk_text_buffer_insert_at_cursor(gtk_text_view_get_buffer(GTK_TEXT_VIEW(txv_console)), "Instrument shutter open\n", -1);
}

void instrshutt_shut(gpointer txv_console)
{
  gtk_text_buffer_insert_at_cursor(gtk_text_view_get_buffer(GTK_TEXT_VIEW(txv_console)), "Instrument shutter shut\n", -1);
}

void instrshutt_fail(gpointer txv_console, const gchar *msg)
{
  char text[256];
  sprintf(text, "Instrument shutter failure: %s\n", msg);
  gtk_text_buffer_insert_at_cursor(gtk_text_view_get_buffer(GTK_TEXT_VIEW(txv_console)), text ,-1);
}
