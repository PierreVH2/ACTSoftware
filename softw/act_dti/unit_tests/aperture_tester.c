...
gboolean plc_stat_update(GIOChannel *plc_chan, GIOCondition condition, gpointer aperture);
void aperture_cent(gpointer txv_console);
void aperture_move(gpointer txv_console);
void proc_complete(gpointer txv_console, gint status);
void aperture_fail(gpointer txv_console, const gchar *msg);
...
int main (int argc, char ** argv)
{
  ...
  GtkWidget *aperture = aperture_new (&plc_stat, plc_chan);
  gtk_table_attach(GTK_TABLE(box_main), aperture, 0, 3, 2, 3, GTK_FILL|GTK_EXPAND, GTK_SHRINK, 5, 5);
  g_signal_connect_swapped(G_OBJECT(aperture), "aperture-cent", G_CALLBACK(aperture_cent), txv_console);
  g_signal_connect_swapped(G_OBJECT(aperture), "aperture-move", G_CALLBACK(aperture_move), txv_console);
  g_signal_connect_swapped(G_OBJECT(aperture), "proc-complete", G_CALLBACK(proc_complete), txv_console);
  g_signal_connect_swapped(G_OBJECT(aperture), "aperture-fail", G_CALLBACK(aperture_fail), txv_console);
  g_signal_connect_swapped(G_OBJECT(aperture), "plc-comm-stat", G_CALLBACK(plc_comm_stat), txv_console);
  g_signal_connect_swapped(G_OBJECT(aperture), "user-interact", G_CALLBACK(user_interact), txv_console);
  ...
  g_object_set_data(send_msg_objects, "aperture", aperture);
  ...
  int plc_watch_id = g_io_add_watch(plc_chan, G_IO_IN, plc_stat_update, aperture);
...
void send_msg(gpointer user_data)
{
  ...
  GtkWidget *aperture = GTK_WIDGET(g_object_get_data(user_data, "aperture"));
  ...
  aperture_process_msg(aperture, &msg);
}
...
gboolean plc_stat_update(GIOChannel *plc_chan, GIOCondition condition, gpointer aperture)
{
  ...
  aperture_update(GTK_WIDGET(aperture), &new_stat);
...
void aperture_cent(gpointer txv_console)
{
  gtk_text_buffer_insert_at_cursor(gtk_text_view_get_buffer(GTK_TEXT_VIEW(txv_console)), "Aperture centred\n", -1);
}

void aperture_move(gpointer txv_console)
{
  gtk_text_buffer_insert_at_cursor(gtk_text_view_get_buffer(GTK_TEXT_VIEW(txv_console)), "Aperture moving\n", -1);
}
...
{
  char text[256];
  sprintf(text, "Aperture failure: %s\n", msg);
  gtk_text_buffer_insert_at_cursor(gtk_text_view_get_buffer(GTK_TEXT_VIEW(txv_console)), text ,-1);
}
