...
gboolean plc_stat_update(GIOChannel *plc_chan, GIOCondition condition, gpointer dropout);
void dropout_open(gpointer txv_console);
void dropout_closed(gpointer txv_console);
void dropout_move(gpointer txv_console);
void dropout_fail(gpointer txv_console, const gchar *msg);
...
void toggle_sensitive(GtkWidget *btn_sensitive, gpointer dropout);
...
int main (int argc, char ** argv)
{
  ...
  GtkWidget *dropout = dropout_new (&plc_stat, plc_chan);
  gtk_table_attach(GTK_TABLE(box_main), dropout, 0, 3, 3, 4, GTK_FILL|GTK_EXPAND, GTK_SHRINK, 5, 5);
  g_signal_connect_swapped(G_OBJECT(dropout), "dropout-open", G_CALLBACK(dropout_open), txv_console);
  g_signal_connect_swapped(G_OBJECT(dropout), "dropout-closed", G_CALLBACK(dropout_closed), txv_console);
  g_signal_connect_swapped(G_OBJECT(dropout), "dropout-move", G_CALLBACK(dropout_move), txv_console);
  g_signal_connect_swapped(G_OBJECT(dropout), "dropout-fail", G_CALLBACK(dropout_fail), txv_console);
  g_signal_connect_swapped(G_OBJECT(dropout), "proc-complete", G_CALLBACK(proc_complete), txv_console);
  g_signal_connect_swapped(G_OBJECT(dropout), "plc-comm-stat", G_CALLBACK(plc_comm_stat), txv_console);
  g_signal_connect_swapped(G_OBJECT(dropout), "user-interact", G_CALLBACK(user_interact), txv_console);
  ...
  g_signal_connect(G_OBJECT(btn_sensitive), "toggled", G_CALLBACK(toggle_sensitive), dropout);
  ...
  g_object_set_data(send_msg_objects, "dropout", dropout);
  ...
  int plc_watch_id = g_io_add_watch(plc_chan, G_IO_IN, plc_stat_update, dropout);
...
void send_msg(gpointer user_data)
{
  ...
  GtkWidget *dropout = GTK_WIDGET(g_object_get_data(user_data, "dropout"));
  ...
  console_scroll_to_end(txv_console);
  dropout_process_msg(dropout, &msg);
}
...
gboolean plc_stat_update(GIOChannel *plc_chan, GIOCondition condition, gpointer dropout)
{
  ...
  dropout_update(GTK_WIDGET(dropout), &new_stat);
...
void dropout_open(gpointer txv_console)
{
  gtk_text_buffer_insert_at_cursor(gtk_text_view_get_buffer(GTK_TEXT_VIEW(txv_console)), "Dropout open\n", -1);
  console_scroll_to_end(txv_console);
}

void dropout_closed(gpointer txv_console)
{
  gtk_text_buffer_insert_at_cursor(gtk_text_view_get_buffer(GTK_TEXT_VIEW(txv_console)), "Dropout closed\n", -1);
  console_scroll_to_end(txv_console);
}

void dropout_move(gpointer txv_console)
{
  gtk_text_buffer_insert_at_cursor(gtk_text_view_get_buffer(GTK_TEXT_VIEW(txv_console)), "Dropout moving\n", -1);
  console_scroll_to_end(txv_console);
}
...
{
  char text[256];
  sprintf(text, "Dropout failure: %s\n", msg);
  gtk_text_buffer_insert_at_cursor(gtk_text_view_get_buffer(GTK_TEXT_VIEW(txv_console)), text ,-1);
  console_scroll_to_end(txv_console);
}
...
void toggle_sensitive(GtkWidget *btn_sensitive, gpointer dropout)
{
  dropout_set_sensitive(GTK_WIDGET(dropout), gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(btn_sensitive)));
}
