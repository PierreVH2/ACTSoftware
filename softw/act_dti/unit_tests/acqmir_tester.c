...
void console_text_insert(GtkWidget *txv_console);

gboolean plc_stat_update(GIOChannel *plc_chan, GIOCondition condition, gpointer acqmir);
void acqmir_view(gpointer txv_console);
void acqmir_meas(gpointer txv_console);
void acqmir_move(gpointer txv_console);
void proc_complete(gpointer txv_console, gint status);
void acqmir_fail(gpointer txv_console, const gchar *msg);
...
int main (int argc, char ** argv)
{
  ...
  GtkWidget *acqmir = acqmir_new (&plc_stat, plc_chan);
  gtk_table_attach(GTK_TABLE(box_main), acqmir, 0, 3, 2, 3, GTK_FILL|GTK_EXPAND, GTK_SHRINK, 5, 5);
  g_signal_connect_swapped(G_OBJECT(acqmir), "acqmir-view", G_CALLBACK(acqmir_view), txv_console);
  g_signal_connect_swapped(G_OBJECT(acqmir), "acqmir-meas", G_CALLBACK(acqmir_meas), txv_console);
  g_signal_connect_swapped(G_OBJECT(acqmir), "acqmir-move", G_CALLBACK(acqmir_move), txv_console);
  g_signal_connect_swapped(G_OBJECT(acqmir), "proc-complete", G_CALLBACK(proc_complete), txv_console);
  g_signal_connect_swapped(G_OBJECT(acqmir), "acqmir-fail", G_CALLBACK(acqmir_fail), txv_console);
  g_signal_connect_swapped(G_OBJECT(acqmir), "plc-comm-stat", G_CALLBACK(plc_comm_stat), txv_console);
  g_signal_connect_swapped(G_OBJECT(acqmir), "user-interact", G_CALLBACK(user_interact), txv_console);
  ...
  g_object_set_data(send_msg_objects, "acqmir", acqmir);
  ...
  g_signal_connect(G_OBJECT(txv_console), "insert-at-cursor", G_CALLBACK(console_text_insert), NULL);
  int plc_watch_id = g_io_add_watch(plc_chan, G_IO_IN, plc_stat_update, acqmir);
...
void send_msg(gpointer user_data)
{
  ...
  GtkWidget *acqmir = GTK_WIDGET(g_object_get_data(user_data, "acqmir"));
  ...
  switch (msg.mtype)
  {
    ...
    case MT_COORD:
    {
      ...
      convert_D_DMS_alt(tmpdec_d, &msg_coord->alt);
      convert_D_DMS_azm(tmpdec_d, &msg_coord->azm);
      break;
    }
  ...
  acqmir_process_msg(acqmir, &msg);
}

void console_text_insert(GtkWidget *txv_console)
{
  GtkTextIter iter;
  gtk_text_buffer_get_end_iter(gtk_text_view_get_buffer(GTK_TEXT_VIEW(txv_console)), &iter);
  gtk_text_view_scroll_to_iter(GTK_TEXT_VIEW(txv_console), &iter, 0.0, FALSE, 0.0, 0.0);
}
...
gboolean plc_stat_update(GIOChannel *plc_chan, GIOCondition condition, gpointer acqmir)
{
  ...
  acqmir_update(GTK_WIDGET(acqmir), &new_stat);
...
void acqmir_view(gpointer txv_console)
{
  gtk_text_buffer_insert_at_cursor(gtk_text_view_get_buffer(GTK_TEXT_VIEW(txv_console)), "Mirror in View position\n", -1);
}

void acqmir_meas(gpointer txv_console)
{
  gtk_text_buffer_insert_at_cursor(gtk_text_view_get_buffer(GTK_TEXT_VIEW(txv_console)), "Mirror in Measure position\n", -1);
}

void acqmir_move(gpointer txv_console)
{
  gtk_text_buffer_insert_at_cursor(gtk_text_view_get_buffer(GTK_TEXT_VIEW(txv_console)), "Mirror moving\n", -1);
}

void proc_complete(gpointer txv_console, gint status)
{
  char text[256];
  sprintf(text, "Processing complete, status: %d\n", status);
  gtk_text_buffer_insert_at_cursor(gtk_text_view_get_buffer(GTK_TEXT_VIEW(txv_console)), text, -1);
}
