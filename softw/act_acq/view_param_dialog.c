...
static gboolean grid_set_active(GtkWidget *cmb_grid_type, gint active_grid);
...
GtkWidget *view_param_dialog_new(GtkWidget *parent, GtkWidget *imgdisp)
{
  ...
  gtk_dialog_add_buttons (GTK_DIALOG(objs), GTK_STOCK_CLOSE, GTK_RESPONSE_CLOSE, GTK_STOCK_REVERT_TO_SAVED, GTK_RESPONSE_CANCEL, NULL);
  ...
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(objs->btn_flip_ns), objs->orig_flip_ns);
  g_signal_connect(G_OBJECT(objs->btn_flip_ns),"toggled",G_CALLBACK(toggle_flip_ns),imgdisp);
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(objs->btn_flip_ew), objs->orig_flip_ew);
  g_signal_connect(G_OBJECT(objs->btn_flip_ew),"toggled",G_CALLBACK(toggle_flip_ew),imgdisp);

  grid_set_active(objs->cmb_grid_type, objs->orig_grid_type);
  g_signal_connect(G_OBJECT(objs->cmb_grid_type),"changed",G_CALLBACK(grid_type_change),imgdisp);
  gtk_range_set_value(GTK_RANGE(objs->scl_grid_x), log10(objs->orig_grid_spacing_x)/log10(2.0));
  g_signal_connect(G_OBJECT(objs->scl_grid_x),"value-changed",G_CALLBACK(grid_scale_x_change),imgdisp);
  gtk_range_set_value(GTK_RANGE(objs->scl_grid_y), log10(objs->orig_grid_spacing_y)/log10(2.0));
  g_signal_connect(G_OBJECT(objs->scl_grid_y),"value-changed",G_CALLBACK(grid_scale_y_change),imgdisp);
  
  gtk_widget_set_sensitive(objs->cmb_lut, FALSE);
  gtk_range_set_value(GTK_RANGE(objs->scl_faint), objs->orig_faint);
  g_signal_connect(G_OBJECT(objs->scl_faint), "value-changed", G_CALLBACK(faint_changed), imgdisp);
  gtk_range_set_value(GTK_RANGE(objs->scl_bright), objs->orig_bright);
  g_signal_connect(G_OBJECT(objs->scl_bright), "value-changed", G_CALLBACK(bright_changed), imgdisp);
...
static void instance_init(GtkWidget *view_param_dialog)
{
  ...
  gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(objs->cmb_grid_type), renderer, TRUE);
  gtk_cell_layout_add_attribute(GTK_CELL_LAYOUT(objs->cmb_grid_type), renderer, "text", GRIDSTORE_NAME);
  gtk_combo_box_set_model(GTK_COMBO_BOX(objs->cmb_grid_type), GTK_TREE_MODEL(gridstore));
  ...
  objs->scl_grid_x = gtk_hscale_new_with_range(-4.0, 4.0, 0.1);
  gtk_scale_set_draw_value(GTK_SCALE(objs->scl_grid_x), TRUE);
  gtk_scale_set_value_pos (GTK_SCALE(objs->scl_grid_x), GTK_POS_RIGHT);
  gtk_table_attach(GTK_TABLE(box_content), objs->scl_grid_x, 3, 6, 2, 3, GTK_FILL|GTK_EXPAND, GTK_FILL|GTK_EXPAND, TABLE_PADDING, TABLE_PADDING);
  ...
  objs->scl_grid_y = gtk_hscale_new_with_range(-4.0, 4.0, 0.1);
  gtk_scale_set_draw_value(GTK_SCALE(objs->scl_grid_y), TRUE);
  gtk_scale_set_value_pos (GTK_SCALE(objs->scl_grid_y), GTK_POS_RIGHT);
  gtk_table_attach(GTK_TABLE(box_content), objs->scl_grid_y, 3, 6, 3, 4, GTK_FILL|GTK_EXPAND, GTK_FILL|GTK_EXPAND, TABLE_PADDING, TABLE_PADDING);
  
  objs->cmb_lut = gtk_combo_box_new();
  gtk_table_attach(GTK_TABLE(box_content), objs->cmb_lut, 0, 2, 5, 7, GTK_FILL|GTK_EXPAND, GTK_FILL|GTK_EXPAND, TABLE_PADDING, TABLE_PADDING);
  ...
  gtk_scale_set_draw_value(GTK_SCALE(objs->scl_faint), TRUE);
  gtk_scale_set_value_pos (GTK_SCALE(objs->scl_faint), GTK_POS_RIGHT);
  gtk_table_attach(GTK_TABLE(box_content), objs->scl_faint, 3, 6, 5, 6, GTK_FILL|GTK_EXPAND, GTK_FILL|GTK_EXPAND, TABLE_PADDING, TABLE_PADDING);
  ...
  gtk_scale_set_draw_value(GTK_SCALE(objs->scl_bright), TRUE);
  gtk_scale_set_value_pos (GTK_SCALE(objs->scl_bright), GTK_POS_RIGHT);
  gtk_table_attach(GTK_TABLE(box_content), objs->scl_bright, 3, 6, 6, 7, GTK_FILL|GTK_EXPAND, GTK_FILL|GTK_EXPAND, TABLE_PADDING, TABLE_PADDING);
}
...
static void grid_scale_x_change(GtkWidget *scl_grid_x, gpointer imgdisp)
{
  ...
  gfloat cur_spacing_x, cur_spacing_y;
  grid_get_cur(GTK_WIDGET(imgdisp), &cur_grid_type, &cur_spacing_x, &cur_spacing_y);;
  imgdisp_set_grid(GTK_WIDGET(imgdisp), cur_grid_type, pow(2.0,gtk_range_get_value(GTK_RANGE(scl_grid_x))), cur_spacing_y);
}
...
static void grid_scale_y_change(GtkWidget *scl_grid_y, gpointer imgdisp)
{
  ...
  gfloat cur_spacing_x, cur_spacing_y;
  grid_get_cur(GTK_WIDGET(imgdisp), &cur_grid_type, &cur_spacing_x, &cur_spacing_y);;
  imgdisp_set_grid(GTK_WIDGET(imgdisp), cur_grid_type, cur_spacing_x, pow(2.0,gtk_range_get_value(GTK_RANGE(scl_grid_y))));
}
...
static gboolean grid_set_active(GtkWidget *cmb_grid_type, gint active_grid)
{
  ...
  if (!gtk_tree_model_get_iter_first(model, &iter))
    return FALSE;
  gtk_tree_model_get(model, &iter, GRIDSTORE_TYPE, &tmp_grid, -1);
  while (active_grid != tmp_grid)
  {
    if (!gtk_tree_model_iter_next(model, &iter))
      return FALSE;
    gtk_tree_model_get(model, &iter, GRIDSTORE_TYPE, &tmp_grid, -1);
  }
  gtk_combo_box_set_active_iter (GTK_COMBO_BOX(cmb_grid_type), &iter);
  return TRUE;
}
