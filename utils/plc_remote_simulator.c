...
struct command_objects
{
  ...
  GtkWidget *evb_foc_go, *evb_foc_out, *evb_foc_in, *evb_foc_init;
...
void send_stat()
{
  sprintf(&G_plc_stat[STAT_FCS_OFFS], CNTR_FCS_TERM_FMT, calc_fcs(G_plc_stat, STAT_FCS_OFFS));
  int bytes_comm=0, i, tmpbytes;
  for (i=0; (i<PLC_RETRIES) && (bytes_comm < PLC_STAT_RESP_LEN); i++)
  {
    usleep(1000);
    tmpbytes = write(G_tty_fd, &G_plc_stat[bytes_comm],  PLC_STAT_RESP_LEN-bytes_comm);
    if ((tmpbytes < 0) && (errno != EAGAIN))
      break;
    if (tmpbytes > 0)
      bytes_comm += tmpbytes;
  }
  if (bytes_comm < PLC_STAT_RESP_LEN)
...
{
  int bytes_comm=0, i, tmpbytes;
  char buf[PLC_CMD_RESP_LEN+1];
  snprintf(buf, PLC_CMD_RESP_LEN, PLC_CMD_RESP);
  for (i=0; (i<PLC_RETRIES) && (bytes_comm < PLC_CMD_RESP_LEN); i++)
  {
    usleep(1000);
    tmpbytes = write(G_tty_fd, &buf[bytes_comm],  PLC_CMD_RESP_LEN-bytes_comm);
    if ((tmpbytes < 0) && (errno != EAGAIN))
      break;
    if (tmpbytes > 0)
      bytes_comm += tmpbytes;
  }
  if (bytes_comm < PLC_CMD_RESP_LEN)
    fprintf(stderr, "Error sending command acknowledge.\n");
}

void process_cmd(const char *cmd, int cmd_len)
{
  (void)cmd_len;
  
  char tmpstr[10];
  int tmp_val;
  gtk_label_set_text(GTK_LABEL(G_cmd_objs.lbl_cmd_str), cmd);
  snprintf(tmpstr, CNTR_DOME_POS_LEN+1, &cmd[CNTR_DOME_POS_OFFS]);
  gtk_label_set_text(GTK_LABEL(G_cmd_objs.lbl_dome_pos), tmpstr);
  snprintf(tmpstr, CNTR_DOME_FLOP_LEN+1, &cmd[CNTR_DOME_MAX_FLOP_OFFS]);
  gtk_label_set_text(GTK_LABEL(G_cmd_objs.lbl_dome_max_flop), tmpstr);
  snprintf(tmpstr, CNTR_DOME_FLOP_LEN+1, &cmd[CNTR_DOME_MIN_FLOP_OFFS]);
  gtk_label_set_text(GTK_LABEL(G_cmd_objs.lbl_dome_min_flop), tmpstr);
  snprintf(tmpstr, CNTR_DOME_POS_LEN+1, &cmd[CNTR_DOME_OFFS_OFFS]);
  gtk_label_set_text(GTK_LABEL(G_cmd_objs.lbl_dome_offs), tmpstr);
  tmpstr[0] = cmd[CNTR_FOCUS_REG_OFFS] == CNTR_FOCUS_REG_OUT_VAL ? '-' : ' ';
  snprintf(&tmpstr[1], CNTR_FOCUS_POS_LEN+1, &cmd[CNTR_FOCUS_POS_OFFS]);
  gtk_label_set_text(GTK_LABEL(G_cmd_objs.lbl_foc_pos), tmpstr);
  snprintf(tmpstr, 2, "%c", cmd[CNTR_FILT_NUM_OFFS]);
  gtk_label_set_text(GTK_LABEL(G_cmd_objs.lbl_filt_num), tmpstr);
  snprintf(tmpstr, 2, "%c", cmd[CNTR_APER_NUM_OFFS]);
  gtk_label_set_text(GTK_LABEL(G_cmd_objs.lbl_aper_num), tmpstr);
  tmp_val = hexchar2int(cmd[CNTR_INSTR_SHUTT_OFFS]);
  if ((tmp_val & CNTR_INSTR_SHUTT_MASK) > 0)
    gtk_widget_modify_bg(G_cmd_objs.evb_instr_shutt,GTK_STATE_NORMAL,&G_col_green);
  else
    gtk_widget_modify_bg(G_cmd_objs.evb_instr_shutt,GTK_STATE_NORMAL,NULL);
  if ((tmp_val & CNTR_ACQRESET_MASK) > 0)
    gtk_widget_modify_bg(G_cmd_objs.evb_acqreset,GTK_STATE_NORMAL,&G_col_green);
  else
    gtk_widget_modify_bg(G_cmd_objs.evb_acqreset,GTK_STATE_NORMAL,NULL);
  if ((tmp_val & CNTR_WATCHDOG_MASK) > 0)
    gtk_widget_modify_bg(G_cmd_objs.evb_watchdog,GTK_STATE_NORMAL,&G_col_green);
  else
    gtk_widget_modify_bg(G_cmd_objs.evb_watchdog,GTK_STATE_NORMAL,NULL);
  tmp_val = hexchar2int(cmd[CNTR_DROPOUT_OFFS]);
  if ((tmp_val & CNTR_DSHUTT_OPEN_MASK) > 0)
    gtk_widget_modify_bg(G_cmd_objs.evb_dropout_open,GTK_STATE_NORMAL,&G_col_green);
  else
    gtk_widget_modify_bg(G_cmd_objs.evb_dropout_open,GTK_STATE_NORMAL,NULL);
  if ((tmp_val & CNTR_DSHUTT_CLOSE_MASK) > 0)
    gtk_widget_modify_bg(G_cmd_objs.evb_dropout_close,GTK_STATE_NORMAL,&G_col_green);
  else
    gtk_widget_modify_bg(G_cmd_objs.evb_dropout_close,GTK_STATE_NORMAL,NULL);
  tmp_val = hexchar2int(cmd[CNTR_SHUTTER_OFFS]);
  if ((tmp_val & CNTR_DSHUTT_OPEN_MASK) > 0)
    gtk_widget_modify_bg(G_cmd_objs.evb_shutter_open,GTK_STATE_NORMAL,&G_col_green);
  else
    gtk_widget_modify_bg(G_cmd_objs.evb_shutter_open,GTK_STATE_NORMAL,NULL);
  if ((tmp_val & CNTR_DSHUTT_CLOSE_MASK) > 0)
    gtk_widget_modify_bg(G_cmd_objs.evb_shutter_close,GTK_STATE_NORMAL,&G_col_green);
  else
    gtk_widget_modify_bg(G_cmd_objs.evb_shutter_close,GTK_STATE_NORMAL,NULL);
  tmp_val = hexchar2int(cmd[CNTR_DOME_STAT_OFFS]);
  if ((tmp_val & CNTR_DOME_GUIDE_MASK) > 0)
    gtk_widget_modify_bg(G_cmd_objs.evb_dome_guide,GTK_STATE_NORMAL,&G_col_green);
  else
    gtk_widget_modify_bg(G_cmd_objs.evb_dome_guide,GTK_STATE_NORMAL,NULL);
  if ((tmp_val & CNTR_DOME_MOVE_MASK) > 0)
    gtk_widget_modify_bg(G_cmd_objs.evb_dome_move,GTK_STATE_NORMAL,&G_col_green);
  else
    gtk_widget_modify_bg(G_cmd_objs.evb_dome_move,GTK_STATE_NORMAL,NULL);
  if ((tmp_val & CNTR_DOME_MOVE_LEFT_MASK) > 0)
    gtk_widget_modify_bg(G_cmd_objs.evb_dome_move_left,GTK_STATE_NORMAL,&G_col_green);
  else
    gtk_widget_modify_bg(G_cmd_objs.evb_dome_move_left,GTK_STATE_NORMAL,NULL);
  if ((tmp_val & CNTR_DOME_SET_OFFS_MASK) > 0)
    gtk_widget_modify_bg(G_cmd_objs.evb_set_dome_offs,GTK_STATE_NORMAL,&G_col_green);
  else
    gtk_widget_modify_bg(G_cmd_objs.evb_set_dome_offs,GTK_STATE_NORMAL,NULL);
  if ((tmp_val & CNTR_DOME_GUIDE_MASK) > 0)
    gtk_widget_modify_bg(G_cmd_objs.evb_dome_guide,GTK_STATE_NORMAL,&G_col_green);
  else
    gtk_widget_modify_bg(G_cmd_objs.evb_dome_guide,GTK_STATE_NORMAL,NULL);
  tmp_val = hexchar2int(cmd[CNTR_FOCUS_OFFS]);
  if ((tmp_val & CNTR_FOC_GO_MASK) > 0)
    gtk_widget_modify_bg(G_cmd_objs.evb_foc_go,GTK_STATE_NORMAL,&G_col_green);
  else
    gtk_widget_modify_bg(G_cmd_objs.evb_foc_go,GTK_STATE_NORMAL,NULL);
  if ((tmp_val & CNTR_FOC_OUT_MASK) > 0)
    gtk_widget_modify_bg(G_cmd_objs.evb_foc_out,GTK_STATE_NORMAL,&G_col_green);
  else
    gtk_widget_modify_bg(G_cmd_objs.evb_foc_out,GTK_STATE_NORMAL,NULL);
  if ((tmp_val & CNTR_FOC_IN_MASK) > 0)
    gtk_widget_modify_bg(G_cmd_objs.evb_foc_in,GTK_STATE_NORMAL,&G_col_green);
  else
    gtk_widget_modify_bg(G_cmd_objs.evb_foc_in,GTK_STATE_NORMAL,NULL);
  tmp_val = hexchar2int(cmd[CNTR_APER_STAT_OFFS]);
  if ((tmp_val & CNTR_FOC_INIT_MASK) > 0)
    gtk_widget_modify_bg(G_cmd_objs.evb_foc_init,GTK_STATE_NORMAL,&G_col_green);
  else
    gtk_widget_modify_bg(G_cmd_objs.evb_foc_init,GTK_STATE_NORMAL,NULL);
  // ??? forgot about foc_reset
  if ((tmp_val & CNTR_FOC_INIT_MASK) > 0)
    gtk_widget_modify_bg(G_cmd_objs.evb_foc_init,GTK_STATE_NORMAL,&G_col_green);
  else
    gtk_widget_modify_bg(G_cmd_objs.evb_foc_init,GTK_STATE_NORMAL,NULL);
  if ((tmp_val & CNTR_APER_INIT_MASK) > 0)
    gtk_widget_modify_bg(G_cmd_objs.evb_aper_init,GTK_STATE_NORMAL,&G_col_green);
  else
    gtk_widget_modify_bg(G_cmd_objs.evb_aper_init,GTK_STATE_NORMAL,NULL);
  if ((tmp_val & CNTR_APER_GO_MASK) > 0)
    gtk_widget_modify_bg(G_cmd_objs.evb_aper_go,GTK_STATE_NORMAL,&G_col_green);
  else
    gtk_widget_modify_bg(G_cmd_objs.evb_aper_go,GTK_STATE_NORMAL,NULL);
  if ((tmp_val & CNTR_APER_RESET_MASK) > 0)
    gtk_widget_modify_bg(G_cmd_objs.evb_aper_reset,GTK_STATE_NORMAL,&G_col_green);
  else
    gtk_widget_modify_bg(G_cmd_objs.evb_aper_reset,GTK_STATE_NORMAL,NULL);
  tmp_val = hexchar2int(cmd[CNTR_FILT_STAT_OFFS]);
  if ((tmp_val & CNTR_APER_INIT_MASK) > 0)
    gtk_widget_modify_bg(G_cmd_objs.evb_filt_init,GTK_STATE_NORMAL,&G_col_green);
  else
    gtk_widget_modify_bg(G_cmd_objs.evb_filt_init,GTK_STATE_NORMAL,NULL);
  if ((tmp_val & CNTR_APER_GO_MASK) > 0)
    gtk_widget_modify_bg(G_cmd_objs.evb_filt_go,GTK_STATE_NORMAL,&G_col_green);
  else
    gtk_widget_modify_bg(G_cmd_objs.evb_filt_go,GTK_STATE_NORMAL,NULL);
  if ((tmp_val & CNTR_APER_RESET_MASK) > 0)
    gtk_widget_modify_bg(G_cmd_objs.evb_filt_reset,GTK_STATE_NORMAL,&G_col_green);
  else
    gtk_widget_modify_bg(G_cmd_objs.evb_filt_reset,GTK_STATE_NORMAL,NULL);
  tmp_val = hexchar2int(cmd[CNTR_ACQMIR_EHT_OFFS]);
  if ((tmp_val & CNTR_EHT_LO_MASK) > 0)
    gtk_widget_modify_bg(G_cmd_objs.evb_eht_lo,GTK_STATE_NORMAL,&G_col_green);
  else
    gtk_widget_modify_bg(G_cmd_objs.evb_eht_lo,GTK_STATE_NORMAL,NULL);
  if ((tmp_val & CNTR_EHT_HI_MASK) > 0)
    gtk_widget_modify_bg(G_cmd_objs.evb_eht_hi,GTK_STATE_NORMAL,&G_col_green);
  else
    gtk_widget_modify_bg(G_cmd_objs.evb_eht_hi,GTK_STATE_NORMAL,NULL);
  if ((tmp_val & CNTR_ACQMIR_RESET_MASK) > 0)
    gtk_widget_modify_bg(G_cmd_objs.evb_acqmir_reset,GTK_STATE_NORMAL,&G_col_green);
  else
    gtk_widget_modify_bg(G_cmd_objs.evb_acqmir_reset,GTK_STATE_NORMAL,NULL);
  if ((tmp_val & CNTR_ACQMIR_INBEAM_MASK) > 0)
    gtk_widget_modify_bg(G_cmd_objs.evb_acqmir_inbeam,GTK_STATE_NORMAL,&G_col_green);
  else
    gtk_widget_modify_bg(G_cmd_objs.evb_acqmir_inbeam,GTK_STATE_NORMAL,NULL);
}

void process_msg(char *buf, int len)
{
  if ((buf[0] != '@') || (buf[len-1] != '*'))
  {
    fprintf(stderr, "Invalid string received: %s (%d)\n", buf, len);
    return;
  }
  
  if (strncmp(buf, PLC_STAT_REQ, PLC_STAT_REQ_LEN-1) == 0)
    send_stat();
  else if (strncmp(buf, PLC_CMD_HEAD, PLC_CMD_HEAD_LEN-1) == 0)
  {
    process_cmd(buf, len);
    send_cmd_ack();
  }
...
void process_chunk(char *buf, int len)
{
  ...
  {
    G_bytes_recvd = end_pos-start_pos;
    if (G_bytes_recvd > (int)sizeof(G_recv_buf)-1)
    {
      printf("Strange - received message that is larger than %d. This must be an invalid message. Ignoring.\n", sizeof(G_recv_buf));
      G_bytes_recvd = 0;
      return;
    }
    memcpy(G_recv_buf, &buf[start_pos], G_bytes_recvd);
    G_recv_buf[G_bytes_recvd] = '\0';
    process_msg(G_recv_buf, G_bytes_recvd);
    if (len > G_bytes_recvd)
      process_chunk(&buf[G_bytes_recvd], len-G_bytes_recvd);
    return;
  }
  // Check if end of message (*) in buf - process only up to *, then handle the rest recursively
  if (end_pos >= 0)
  {
    if (end_pos+G_bytes_recvd > (int)sizeof(G_recv_buf)-1)
    {
  ...
  if (start_pos >= 0)
    ...
    start_pos = 0;
  memcpy(&G_recv_buf[G_bytes_recvd], &buf[start_pos], len-start_pos);
  G_bytes_recvd += len-start_pos;
}
...
void sigio_handler(int signum)
{
  ...
  bytes_comm = read(G_tty_fd, buf, sizeof(buf));
  if (bytes_comm < 0)
  {
    fprintf(stderr, "Received SIGIO but couldn't read data. Ignoring.\n");
    return;
  }
  process_chunk(buf, bytes_comm);
}
...
int main(int argc, char **argv)
{
  ...
  if (argc != 2)
  {
    fprintf(stderr, "Please give the tty device filename as a parameter (e.g. ""%s /dev/ttyS0"")\n", argv[0]);
    return 1;
  }
  
  G_tty_fd = open(argv[1], O_RDWR | O_NONBLOCK);
  if (G_tty_fd < 0)
  {
  ...
  fcntl(G_tty_fd, F_SETOWN, getpid());
  int oflags = fcntl(G_tty_fd, F_GETFL);
  fcntl(G_tty_fd, F_SETFL, oflags | FASYNC);
  ...
  g_signal_connect_swapped(G_OBJECT(chk_focus_slot), "toggled", G_CALLBACK(set_stat_char), &G_plc_stat[STAT_FOC_STAT_OFFS1]);
  ...
  g_signal_connect_swapped(G_OBJECT(chk_focus_ref), "toggled", G_CALLBACK(set_stat_char), &G_plc_stat[STAT_FOC_STAT_OFFS1]);
  ...
  g_signal_connect_swapped(G_OBJECT(chk_focus_out), "toggled", G_CALLBACK(set_stat_char), &G_plc_stat[STAT_FOC_STAT_OFFS1]);
  ...
  g_signal_connect_swapped(G_OBJECT(chk_focus_in), "toggled", G_CALLBACK(set_stat_char), &G_plc_stat[STAT_FOC_STAT_OFFS1]);
  ...
  g_signal_connect_swapped(G_OBJECT(chk_focus_moving), "toggled", G_CALLBACK(set_stat_char), &G_plc_stat[STAT_FOC_STAT_OFFS2]);
  ...
  g_signal_connect_swapped(G_OBJECT(chk_focus_init), "toggled", G_CALLBACK(set_stat_char), &G_plc_stat[STAT_FOC_STAT_OFFS2]);
  ...
  g_signal_connect_swapped(G_OBJECT(chk_focus_stall), "toggled", G_CALLBACK(set_stat_char), &G_plc_stat[STAT_FOC_STAT_OFFS2]);
  ...
  G_cmd_objs.lbl_cmd_str = gtk_label_new("");
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.lbl_cmd_str, 0,8,8,9, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  ...
  G_cmd_objs.lbl_dome_pos = gtk_label_new("");
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.lbl_dome_pos, 1,2,0,1, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  ...
  G_cmd_objs.lbl_dome_offs = gtk_label_new("");
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.lbl_dome_offs, 1,2,1,2, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  gtk_table_attach(GTK_TABLE(box_command), gtk_label_new("Dome offset"), 0,1,1,2, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  G_cmd_objs.lbl_dome_offs = gtk_label_new("");
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.lbl_dome_offs, 1,2,1,2, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  G_cmd_objs.evb_set_dome_offs = gtk_event_box_new();
  gtk_container_add(GTK_CONTAINER(G_cmd_objs.evb_set_dome_offs), gtk_label_new("Set dome offset"));
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.evb_set_dome_offs, 0,2,2,3, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  ...
  G_cmd_objs.lbl_dome_max_flop = gtk_label_new("");
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.lbl_dome_max_flop, 1,2,3,4, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  ...
  G_cmd_objs.lbl_dome_min_flop = gtk_label_new("");
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.lbl_dome_min_flop, 1,2,4,5, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  G_cmd_objs.evb_dome_guide = gtk_event_box_new();
  gtk_container_add(GTK_CONTAINER(G_cmd_objs.evb_dome_guide), gtk_label_new("Dome guide"));
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.evb_dome_guide, 0,2,5,6, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  G_cmd_objs.evb_dome_move = gtk_event_box_new();
  gtk_container_add(GTK_CONTAINER(G_cmd_objs.evb_dome_move), gtk_label_new("Dome move"));
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.evb_dome_move, 0,2,6,7, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  G_cmd_objs.evb_dome_move_left = gtk_event_box_new();
  gtk_container_add(GTK_CONTAINER(G_cmd_objs.evb_dome_move_left), gtk_label_new("Dome move left"));
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.evb_dome_move_left, 0,2,7,8, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  
  G_cmd_objs.evb_acqreset = gtk_event_box_new();
  gtk_container_add(GTK_CONTAINER(G_cmd_objs.evb_acqreset), gtk_label_new("Reset ACQ CCD"));
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.evb_acqreset, 2,4,0,1, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  G_cmd_objs.evb_watchdog = gtk_event_box_new();
  gtk_container_add(GTK_CONTAINER(G_cmd_objs.evb_watchdog), gtk_label_new("Watchdog reset"));
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.evb_watchdog, 2,4,1,2, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  G_cmd_objs.evb_dropout_open = gtk_event_box_new();
  gtk_container_add(GTK_CONTAINER(G_cmd_objs.evb_dropout_open), gtk_label_new("Dropout Open"));
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.evb_dropout_open, 2,4,2,3, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  G_cmd_objs.evb_dropout_close = gtk_event_box_new();
  gtk_container_add(GTK_CONTAINER(G_cmd_objs.evb_dropout_close), gtk_label_new("Dropout Close"));
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.evb_dropout_close, 2,4,3,4, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  G_cmd_objs.evb_shutter_open = gtk_event_box_new();
  gtk_container_add(GTK_CONTAINER(G_cmd_objs.evb_shutter_open), gtk_label_new("Shutter Open"));
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.evb_shutter_open, 2,4,4,5, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  G_cmd_objs.evb_shutter_close = gtk_event_box_new();
  gtk_container_add(GTK_CONTAINER(G_cmd_objs.evb_shutter_close), gtk_label_new("Shutter Close"));
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.evb_shutter_close, 2,4,5,6, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  G_cmd_objs.evb_acqmir_reset = gtk_event_box_new();
  gtk_container_add(GTK_CONTAINER(G_cmd_objs.evb_acqmir_reset), gtk_label_new("Acq mirror reset"));
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.evb_acqmir_reset, 2,4,6,7, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  G_cmd_objs.evb_acqmir_inbeam = gtk_event_box_new();
  gtk_container_add(GTK_CONTAINER(G_cmd_objs.evb_acqmir_inbeam), gtk_label_new("Acq mirror in-beam"));
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.evb_acqmir_inbeam, 2,4,7,8, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);

  G_cmd_objs.evb_instr_shutt = gtk_event_box_new();
  gtk_container_add(GTK_CONTAINER(G_cmd_objs.evb_instr_shutt), gtk_label_new("Instr shutt open"));
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.evb_instr_shutt, 4,6,0,1, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  G_cmd_objs.evb_eht_lo = gtk_event_box_new();
  gtk_container_add(GTK_CONTAINER(G_cmd_objs.evb_eht_lo), gtk_label_new("EHT Low"));
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.evb_eht_lo, 4,6,1,2, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  G_cmd_objs.evb_eht_hi = gtk_event_box_new();
  gtk_container_add(GTK_CONTAINER(G_cmd_objs.evb_eht_hi), gtk_label_new("EHT High"));
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.evb_eht_hi, 4,6,2,3, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  ...
  G_cmd_objs.lbl_foc_pos = gtk_label_new("");
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.lbl_foc_pos, 5,6,3,4, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  G_cmd_objs.evb_foc_go = gtk_event_box_new();
  gtk_container_add(GTK_CONTAINER(G_cmd_objs.evb_foc_go), gtk_label_new("Focus go"));
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.evb_foc_go, 4,6,4,5, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  G_cmd_objs.evb_foc_out = gtk_event_box_new();
  gtk_container_add(GTK_CONTAINER(G_cmd_objs.evb_foc_out), gtk_label_new("Focus out"));
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.evb_foc_out, 4,6,5,6, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  G_cmd_objs.evb_foc_in = gtk_event_box_new();
  gtk_container_add(GTK_CONTAINER(G_cmd_objs.evb_foc_in), gtk_label_new("Focus in"));
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.evb_foc_in, 4,6,6,7, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  G_cmd_objs.evb_foc_init = gtk_event_box_new();
  gtk_container_add(GTK_CONTAINER(G_cmd_objs.evb_foc_init), gtk_label_new("Focus init"));
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.evb_foc_init, 4,6,7,8, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  ...
  G_cmd_objs.lbl_filt_num = gtk_label_new("");
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.lbl_filt_num, 7,8,0,1, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  G_cmd_objs.evb_filt_go = gtk_event_box_new();
  gtk_container_add(GTK_CONTAINER(G_cmd_objs.evb_filt_go), gtk_label_new("Filter go"));
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.evb_filt_go, 6,8,1,2, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  G_cmd_objs.evb_filt_init = gtk_event_box_new();
  gtk_container_add(GTK_CONTAINER(G_cmd_objs.evb_filt_init), gtk_label_new("Filter init"));
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.evb_filt_init, 6,8,2,3, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  G_cmd_objs.evb_filt_reset = gtk_event_box_new();
  gtk_container_add(GTK_CONTAINER(G_cmd_objs.evb_filt_reset), gtk_label_new("Filter reset"));
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.evb_filt_reset, 6,8,3,4, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  ...
  G_cmd_objs.lbl_aper_num = gtk_label_new("");
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.lbl_aper_num, 7,8,4,5, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  G_cmd_objs.evb_aper_go = gtk_event_box_new();
  gtk_container_add(GTK_CONTAINER(G_cmd_objs.evb_aper_go), gtk_label_new("Aperture go"));
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.evb_aper_go, 6,8,5,6, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  G_cmd_objs.evb_aper_init = gtk_event_box_new();
  gtk_container_add(GTK_CONTAINER(G_cmd_objs.evb_aper_init), gtk_label_new("Aperture init"));
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.evb_aper_init, 6,8,6,7, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
  G_cmd_objs.evb_aper_reset = gtk_event_box_new();
  gtk_container_add(GTK_CONTAINER(G_cmd_objs.evb_aper_reset), gtk_label_new("Aperture reset"));
  gtk_table_attach(GTK_TABLE(box_command), G_cmd_objs.evb_aper_reset, 6,8,7,8, GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,TABLE_PADDING,TABLE_PADDING);
